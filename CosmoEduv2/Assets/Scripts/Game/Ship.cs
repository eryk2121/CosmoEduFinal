...
public class Ship : MonoBehaviour
{
    ...
    void Update()
    {
        if (Pause.pause)
        {
            if (DateTime.Now.Ticks - time >= 2000000 && !endPanel.active && !questionPanel.active) //sprawdzanie czy użytkownik ma możliwość gry
            {
                score++;
    ...
    float counter = 160; //zmienna potrzebna do określenia przeźroczystości koloru
    bool raise = true;
    ...
    {
        if (DateTime.Now.Ticks - time3 > 80000000/(PlayerPrefs.GetInt("FreezeUpgrade")+1))  // warunek określający długość efektu zamrożenia
        {
            freeze = false;
        }
    ...
    public void GainCoins() //określenie dodania monet z bonusu
    {
    ...
    public void AddCoins() //dodanie monet z wyniku
    {
