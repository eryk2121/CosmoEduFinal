...
public class Boss : MonoBehaviour
{
    ...
    private void HorizontalMove() //ruch horyzontalny bossa
    {
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("Missile"))
        {
            ...
            if (finalPosition) //warunek w którym sprawdzamy czy mozna juz uderzyc bossa
            {
    ...
    {
        if (DateTime.Now.Ticks - superShotTime>= 90000000 &&  finalPosition && (gameObject.transform.position.x > (Screen.width / 2 - 10) && gameObject.transform.position.x < (Screen.width / 2 + 10))) //warunek określający czas oraz miejsce z którego może zostać wykonana akcja
        {
            int dir = -6; //zmienna pomocnicza okreslajaca kierunek wystrzalu
            for (int i = 0; i < 5; i++)
            {
                GameObject enemyMissile = Instantiate(enenmyMissile);
